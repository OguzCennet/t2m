...
class bert_embeddings():
    def __init__(self, model_type='bert', model_name='bert-base-uncased', device=None):
    ...
    def get_vectors(self, words):
        ...
        with torch.no_grad():
            ...
            '''
            h1 = pretrained_hiddens[6]
            h2 = pretrained_hiddens[7]

            batch_size, pretrained_seq_length, hidden_size = h1.size(0), h1.size(1), h1.size(2)
            h1_chosen_hiddens = h1.view(-1,hidden_size).index_select(0, selects)
            h2_chosen_hiddens = h2.view(-1,hidden_size).index_select(0, selects)

            embeds = torch.zeros(len(lengths) * max(lengths),hidden_size, device=self.device)
            h1_embeds = embeds.index_copy_(0, copies, h1_chosen_hiddens).view(len(lengths), max(lengths), -1)
            h2_embeds = embeds.index_copy_(0, copies, h2_chosen_hiddens).view(len(lengths), max(lengths), -1)

            concat_hiddens = torch.cat((h1_embeds, h2_embeds), dim=2)

        return concat_hiddens.double(), mask
